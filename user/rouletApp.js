...
function betNow(betArray) {
    $.ajax({
        ...
        dataType: "json",
        success: function (result) {
            $("#header_wallet_balance").text('₹' + result.data.wallet_balance);
		},
		error: function (e) {
...
function spin() {
    ...
    .then(function(result) {
        var winningSpin = result.data.rouletNo;
	...
	setTimeout(function(){
		...
		wager = lastWager;
// if(result.winner == 1) {
    var winAmt = result.data.giveAmt;
    var betTotal = result.data.amount;
    win(winningSpin, winAmt, betTotal);
// }
		gameOver();
        $("#header_wallet_balance").text('₹' + result.data.walletAmt);
	}, 10000);
		...
		notification.setAttribute('id', 'notification');
			...
			nSpan.setAttribute('class', 'nSpan');
				...
				nsWin.setAttribute('class', 'nsWin');
					...
					nsWinBlock = document.createElement('div');
					nsWinBlock.setAttribute('class', 'nsWinBlock');
					nsWinBlock.innerText = 'Payout: ' + (parseInt(winValue) + parseInt(betTotal));
					nsWin.append(nsWinBlock);
				nSpan.append(nsWin);
			notification.append(nSpan);
		container.prepend(notification);
