function deposit(id) {
  $("#payment_gateway_type").val(id);
    ...
    var totalAmt = 0;
    if(getCookie("initialBonus") > 0) {
        var initialBonus = (Number(getCookie("initialBonus")) / 100) * Number(amount);
        // amount = Number(initialBonus) + Number(amount);
        totalAmt = Number(initialBonus) + Number(amount);
        $("#bonusAmt").val(initialBonus);
    }
    $("#deposit_amount").val(amount);
    if(totalAmt > 0) {
        $("#select_amount").text(totalAmt);
    } else {
